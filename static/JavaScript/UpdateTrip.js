function update(){
    if(!document.getElementById("oldTripID").value){
        alert(" old trip ID is required")
        return;
      }
    ...
    const oldid=document.getElementById("oldTripID").value;
    ...
    var trip = { 'oldTripID' :oldid ,'tripID': id, 'source': source, 'tripTime': tripTime, 'destination': dest, 'requiredNumberOfSeats': numberOfSeats, 'availableNumberOfSeats': availableSeats, 'trainID': trainId, 'cost': cost, 'date': date, 'time': time}; // Add all attributes
  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      else if(response.is_exist==false){
        alert("This trip " + oldid + ' not found');
      }
      else if(response.train_exist==false){
        alert("This train " + trainId + ' not found');
      }
      
      
      else{
         alert("Trip Updated successfully " + id);
